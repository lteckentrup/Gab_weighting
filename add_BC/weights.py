...
def weighting(file, var):
    ...
    for m in models:
        ...
        ### Calculate error: simulation minus target
        le_ds_error = ds_SIM - ds_CRU 
        
        ### average of error over time
        bc_term = le_ds_error.mean(dim='Time') 
        
        ### Average bias correction term over entire domain
        BC_term.append(bc_term.mean().values)
        
        ### Correct simulation: simulation minus error
        ds_SIM_bc =  ds_SIM - bc_term 
        
        ### New error: corrected simulation minus observation
        ds_ERROR = ds_SIM_bc - ds_CRU 
        
        ### resulting error matrix: each column = 1 model flattened over all 
        ### timesteps, longitudes, and latitude
